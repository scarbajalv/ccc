...
function setup() {
  ...
  checkbox_time_evolve.position(0.19*canvas_w - 48/2, 0.05*canvas_h-22/2);
  ...
  checkbox_time_evolve.position(0.18*canvas_w - 48/2, 0.05*canvas_h-22/2);
...
function draw() {
	...
	text( "update 8", 0.5*canvas_w, 0.3*canvas_h);
	...
	push();
		...
		// Play
		var aux_width = 70;
		rect(0.066*canvas_w + (0.225*canvas_w - aux_width)/2, 0.05*canvas_h-22/2 - 3, 
			aux_width, 30, 
			borders);
	...
	text("\u03b8Max = ", 0.45*canvas_w, 0.20*canvas_h);
	...
	text(thetaMax_deg.toFixed(0) + "°" + " (" + (thetaMax_deg/180*3.1415).toFixed(2) + " rad)", 
		0.45*canvas_w, 0.20*canvas_h);
	text(period.toFixed(2) + "s", 0.45*canvas_w, 0.24*canvas_h);

	// Other Text
	textAlign(LEFT, CENTER);
	text("t = " + t.toFixed(2) + " s", 0.13*canvas_w, 0.34*canvas_h);
...
function draw_Oscillator(){
	...
	var pendulum_vertex_x = 0.7*canvas_w;
...
function draw_yAxis_x(){
	...
	push();
		noStroke();
		textAlign(RIGHT, CENTER);
		text("-"+amplitude_units, frame_origin_x - canvas_w/100, axis_zero_y + amplitude );
